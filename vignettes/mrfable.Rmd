...
The mrfable R package gives you quick, easy, and handy access to a large amount of foodrcrop data (area, production, and yield) for the federal States of India from 1967 until now. With mrfbale you can automatically download the data from https://eands.dacnet.nic.in/, and correct, clean, and aggregate them. This document provides a quick guide for using the package. 
...
```
The data will be then found in your madrat output folder, which you can find with the following command:
```{r,echo=TRUE,eval=FALSE}
getConfig("outputfolder")
```
## Advanced

The individual steps of data creation will be listed here:

### Download the data
```{r,echo=TRUE,eval=FALSE}
downloadSource("IndiaAPY")
```

### Read in the raw data
```{r,echo=TRUE,eval=FALSE}
mydata <- readSource("IndiaAPY",convert = FALSE)
```

### Read in the raw data and correct mistakes. Deliver the data for the official current list of federal states of India
```{r,echo=TRUE,eval=FALSE}
mydata <- readSource("IndiaAPY",convert = "onlycorrect")
```

### Calculate data for area, production or yield and/or aggregate to a different spatial resolution
(see above: `calcOutput`...)
