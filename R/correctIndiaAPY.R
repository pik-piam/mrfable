...
correctIndiaAPY <- function(x) {
  ...
  season <- state <- variable <- crop <- year <- Value <- unit <- NULL #nolint
  ...
  x %>%
    filter(season %in% c("kharif", "autumn", "winter")) %>%
  ...
  x %>%
    filter(!season %in% c("kharif total", "total")) %>%
    group_by(`season`, `state`, `crop`, `year`, `variable`, `unit`) %>%
    summarise("Value" = sum(`Value`, na.rm = TRUE)) %>%
    mutate(season = "total") %>%
    ungroup() -> tmp
  ...
  x <- toolCountryFill(x, countrylist = as.vector(mapping[, "state"]),
                       no_remove_warning = c("All India", "Daman & Diu"))
