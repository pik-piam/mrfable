...
#    d <- d[-c(grep("State", d[,1]), which(is.na(d[,1]))),] # remove irrelevant rows 
  ...
  .fixdata <- function(a,type1="W",type2=1){
    # use row with years as column names
    if (type1 == "W") {
      colnames(a)<-sub("-.*","",a[3, ]) 
      a <- a[-which(is.na(a[, 1])),]
    ...
    if (type1 == "R") {
      ...
      if (type2 == 1) { # the type2 is chosen depending on which position 
  ...
  for (i in grep("Rice", excelfiles, value = TRUE)) {
    ...
    tmp <- cbind("crop" = "Rice", .fixdata(a,"R"))
    out <- rbind(out, tmp)
  }

  for (i in grep("Wheat", excelfiles, value = TRUE)) {
    suppressMessages(a <- read_excel(i))
    tmp <- cbind("crop" = "Wheat", .fixdata(a,"W"))
    out <- rbind(out, tmp)
  }
  
  for (i in grep("Maize", excelfiles, value = TRUE)) {
    suppressMessages(a <- read_excel(i))
    tmp <- cbind("crop" = "Maize", .fixdata(a[-1,],"R"))
    out <- rbind(out, tmp)
  }
  
  for (i in grep("Bajra", excelfiles, value = TRUE)) {
    suppressMessages(a <- read_excel(i))
    tmp <- cbind("crop" = "Bajra", .fixdata(a[-1,],"R"))
    out <- rbind(out, tmp)
  }
